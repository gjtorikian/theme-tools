...
import { LiquidCheckDefinition, ObjectNode, PropertyNode, Severity, SourceCodeType } from '../../types';
    ...
    const getNestedSettingIds = (node: ObjectNode) => {
      return node.children
        .map((property: PropertyNode) => {
          const settingsNode = property.children.find(
      ...
      async LiquidRawTag() {
        ...
        const settingIds = settingsNode?.children?.map((child: PropertyNode) => {
          ...
          defaultNode?.children
            ...
            ?.value?.children?.map((child: PropertyNode) => ({
              key: child.key.value,
          ...
          [...rootLevelThemeBlocks, ...rootLevelLocalBlocks].flat().map(async ({ node }) => {
            ...
            return settingsNode.children
              .map((settingObj: PropertyNode) => {
                const idNode = settingObj.children.find((prop: { key: { value: string; }; }) => prop.key.value === 'id');
        ...
        const defaultBlocks = defaultNode?.children
          ?.find((child: PropertyNode) => child.key.value === 'blocks')
          ?.value;

        if (defaultBlocks && 'children' in defaultBlocks) {
          defaultBlocks.children.forEach((block: PropertyNode) => {
            const blockTypeObj = block.children?.[0];
            if (blockTypeObj?.value && 'children' in blockTypeObj.value) {
              const settings = blockTypeObj.value.children
                .find((child: PropertyNode) => child.key.value === 'settings');
              
              if (settings?.value && 'children' in settings.value) {
                settings.value.children.forEach((setting: PropertyNode) => {
        ...
        for (const presetBlockSettingId of presetBlockSettingIds) {
          if (!rootLevelBlockSettingIds.flat().some((id) => id === presetBlockSettingId.key)) {
            context.report({
              startIndex: presetBlockSettingId!.start,
              endIndex: presetBlockSettingId!.end,
